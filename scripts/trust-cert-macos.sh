#!/usr/bin/env bash
# Download and install the archive site certificate on macOS.
#
# Universal usage:
#   sudo ./scripts/trust-cert-macos.sh https://192.168.2.136:8443/archive
#   sudo ./scripts/trust-cert-macos.sh 192.168.2.136 8443
#
# Local file:
#   sudo ./scripts/trust-cert-macos.sh /path/to/fullchain.pem
...
usage() {
    cat >&2 <<'EOF'
Usage:
  sudo ./trust-cert-macos.sh <server-base-url>
  sudo ./trust-cert-macos.sh <server-host> [https-port] [root-path]
  sudo ./trust-cert-macos.sh /path/to/fullchain.pem

Examples:
  sudo ./trust-cert-macos.sh https://192.168.2.136:8443/archive
  sudo ./trust-cert-macos.sh 192.168.2.136 8443 /archive
EOF
}
