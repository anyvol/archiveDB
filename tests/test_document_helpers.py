...
def test_compute_stored_file_name_renames_when_designation_differs():
    ...
    assert (
        compute_stored_file_name("ORG.123456.001", "ORG.123456.001 - Report.pdf", "Report")
        == "ORG.123456.001 - Report.pdf"
    )
    assert compute_stored_file_name("ORG.123456.001", "ORG.123456.001 (other).pdf") == (
        "ORG.123456.001 (other).pdf"
    )
...
def test_file_name_matches_designation():
    ...
    assert file_name_matches_designation("ORG.123456.001 - Report.pdf", "ORG.123456.001", "Report")
...
def test_build_upload_rename_message():
    assert build_upload_rename_message("ORG.123456.001", "other.pdf", "Report") == (
        "Файл будет переименован в ORG.123456.001 - Report.pdf"
    )
